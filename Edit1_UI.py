...
class ToDoApp(QWidget):
    ...
    def show_tasks(self, sort_by=None):
        ...
        if sort_by == "unfinished":
            ...
            query += """
                ...
                date(substr(Due_Date, 7, 4) || '-' || substr(Due_Date, 4, 2) || '-' || substr(Due_Date, 1, 2))
                """
    ...
    def toggle_task_completed(self, item):
        ...
        completed = 1 if item.checkState() == Qt.CheckState.Checked else 0
    ...
    def edit_task(self):
        ...
        # Fetch current values from the database
            self.cursor.execute("SELECT task, Due_Date, Priority FROM tasks WHERE id = ?", (task_id,))
            current_task, current_due_date, current_priority = self.cursor.fetchone()

        # Task Text
            new_task, ok_task = QInputDialog.getText(self, "Edit Task", "Modify your task:", text=current_task)
            if not ok_task or not new_task.strip():
                return

        # Due Date
            current_due_display = current_due_date if current_due_date else ""
            new_due_date, ok_due = QInputDialog.getText(self, "Edit Due Date", "Modify due date (DD-MM-YYYY) or leave empty:", text=current_due_display)
            if not ok_due:
                return

            if new_due_date.strip():
                try:
                    datetime.strptime(new_due_date, "%d-%m-%Y")  # Validate the date format
                except ValueError:
                    QMessageBox.warning(self, "Invalid Date", "Please enter a valid date (DD-MM-YYYY).")
                    return
            else:
                new_due_date = None

        # Priority
            priorities = ["Urgent", "Normal", "Low Effort"]
            current_priority_index = priorities.index(current_priority) if current_priority in priorities else 1
            new_priority, ok_priority = QInputDialog.getItem(self, "Edit Priority", "Modify priority:", priorities, current_priority_index, False)
            if not ok_priority:
                return
        ...
        # Update task in the database
            self.cursor.execute(
                ...
                (new_task, new_due_date, new_priority, task_id),
                )
            self.conn.commit()
            self.show_tasks()
        else:
